...
export default function MiniSentiment({ sentiment }: { sentiment: MarketSentiment | null }) {
  ...
  const color = s <= 24 ? "#3182F6" : s <= 44 ? "#84B6FC" : s <= 55 ? "#8B95A1" : s <= 74 ? "#FF7B82" : "#F04452";
  return (
    <div className="hidden items-center gap-2 rounded-xl bg-surface px-3 py-1.5 sm:flex" title="시장 공포·탐욕 지수">
      ...
      <span className="text-sm font-extrabold tabular" style={{ color }}>{s}</span>
      <span className="text-[10px] font-bold" style={{ color }}>{sentiment.label}</span>
    </div>
  );
}
