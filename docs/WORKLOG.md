...
## [T25] `lib/types.ts` 651줄을 도메인별로 나눔
- 일시: 2026-08-27 22:35
- 상태: 완료
- 목적: 알려진 빚 5번. 타입 62개가 한 파일에 있어, 어떤 타입이 어느 화면의 것인지 이름으로만 짐작해야 했다.
- 변경: `frontend/lib/types.ts` 삭제 → `frontend/lib/types/` 9개 파일 + 배럴
- 검증:
  - `npx tsc --noEmit` 무오류. **타입 정의를 옮기는 작업이라 이게 제일 중요한 검사다.**
  - `npm run format:check` / `npm run lint` / `npm test` (`162 passed`) / `npm run build` / `npx playwright test` (`54 passed`) 전부 통과.
- 구성:

| 파일 | 줄 | 내용 |
| --- | --- | --- |
| `common.ts` | 40 | `Period`, `Signal`, `Market`, `DataSourceMetadata`, `PricePoint` 등 전 도메인 공통 |
| `signal.ts` | 69 | 지표, 신호 점수, 리스크 |
| `prediction.ts` | 46 | 단기 예측, 장기 시계, 목표가·매도 시점 |
| `market.ts` | 115 | 수급, 뉴스, 공시, 펀더멘털, 섹터, 심리, 팩터 IC, 학습 신호 |
| `analysis.ts` | 27 | `AnalysisResponse` — 위 넷을 모은 것 |
| `backtest.ts` | 46 | 백테스트 결과와 전략별 요약 |
| `discovery.ts` | 130 | 관심 목록, 대표 종목, 매수 신호 스캔, 급등 탐색 |
| `portfolio.ts` | 100 | 보유 분석, 종목 비교, 최적화·리밸런싱 |
| `retrospective.ts` | 70 | 추천 적중률, 신호 전환 이력 |

- 결정:
  - **배럴(`index.ts`)을 남기고 호출부 31곳은 건드리지 않았다.** `@/lib/types` 라는 이름은 그대로다. 임포트를 도메인별로 갈라 적게 하면, 앞으로 타입 하나가 파일을 옮길 때마다 화면 여러 곳이 같이 깨진다. 파일 나누기의 목적은 읽기 편하자는 것이지 호출부에 위치를 외우게 하는 것이 아니다. T02~T03 에서 서비스 추출할 때 별칭으로 무중단을 만든 것과 같은 판단이다.
  - **의존 방향이 한쪽으로만 흐르게 잘랐다.** `analysis.ts` 만 다른 넷을 import 하고, 나머지는 `common.ts` 만 본다(`market`·`portfolio`·`retrospective` 는 그것도 필요 없었다). 순환이 없으므로 `export *` 로 묶어도 초기화 순서 문제가 생기지 않는다.
  - **내용은 한 글자도 고치지 않았다.** 선언을 통째로 옮기고 주석도 붙어 있던 채로 따라갔다. 옮기면서 같이 고치면, 나중에 타입이 틀렸을 때 이 커밋 어디를 봐야 할지 알 수 없다.
- 다음: T26 컴포넌트 테스트 확대.

## [T26] 표시 컴포넌트 단위 테스트 확대
- 일시: 2026-08-27 23:05
- 상태: 완료
- 목적: 알려진 빚 6번. 컴포넌트 테스트가 표시 계층 일부만 덮고 있었고, 큰 것들이 비어 있었다.
- 변경: `frontend/components/PriceTargetCard.test.tsx` (신규), `frontend/components/ComparePanel.test.tsx` (신규), `frontend/components/SupplyDemandCard.test.tsx` (신규), `frontend/components/ComparePanel.tsx`
- 검증:
  - `npm test` → `Test Files 14 passed (14)`, `Tests 192 passed (192)`. 11 → 14 파일, 162 → 192 케이스.
  - `npm run format:check` / `npm run lint` / `npx playwright test` (`54 passed`) 통과.
- **테스트가 잡은 것**: `ComparePanel` 의 `argbest` 주석은 "동률·전부 null이면 null"이라고 적혀 있는데, 코드는 `v > bestVal` 하나로만 갈라서 **동률이면 먼저 온 종목을 강조**하고 있었다. 표 아래에 "초록 강조 = 항목별 우위"라고 써 있으니, 같은 값에 초록을 칠하면 그 문장이 거짓이 된다. 주석이 맞고 코드가 틀렸다고 보고 코드를 고쳤다.
- 고른 기준:
  - **계산을 하는 컴포넌트만 골랐다.** 백엔드 값을 그대로 찍는 카드는 테스트가 렌더러를 다시 적는 꼴이라 회귀를 못 잡는다. 고른 셋은 각각 자기 판단을 한다 — `PriceTargetCard` 는 horizon 일수를 기간 이름·개월로 바꾸고 시나리오 등락률을 직접 계산한다. `ComparePanel` 은 무엇을 보낼지와 어느 칸을 강조할지를 정한다. `SupplyDemandCard` 는 색으로 의미를 말한다.
  - **`SupplyDemandCard` 는 색을 고정했다.** 한국 관례로 순매수가 빨강(`text-up`), 순매도가 파랑(`text-down`)이다. 토큰 이름만 보면 "오르면 초록"이라는 서구 관례로 되돌리기 쉽고, 되돌려도 화면은 멀쩡하고 숫자도 맞다 — 읽는 사람만 반대로 읽는다. 클래스 이름을 단언하는 게 보통은 나쁜 테스트지만, 여기서는 색 자체가 계약이다.
  - **`+` 부호 붙이기를 셋 다 넣었다.** `up ? "+" : ""` 패턴이 화면 곳곳에 있는데, 조건을 뒤집으면 `+-3.25%` 가 된다.
- **막힌 것**: RTL 의 `getByText` 는 그 요소의 **직계 텍스트 노드만** 이어서 본다. `{up ? "+" : ""}{value.toLocaleString()}` 는 형제 텍스트 노드 둘이라 `"120,000"` 으로는 안 잡히고 `"+120,000"` 이어야 한다. 처음에 이걸 몰라 2건이 깨졌다. 헬퍼에 이유를 적어뒀다.
- 다음: T27 게이트 재확인과 Lighthouse 재측정.

## [T27] 전체 게이트 확인과 Lighthouse 재측정
- 일시: 2026-08-27 23:45
- 상태: 완료
- 목적: T23~T26 이 뭘 바꿨는지 숫자로 확인한다. 백엔드까지 포함해 게이트를 한 번에 돌린다.
- 변경: `README.md` (측정 섹션)
- 검증:
  - 백엔드 `python -m pytest -q` → **`270 passed, 1 warning in 54.82s`**. (`.venv/Scripts/python.exe` 로 돌린다 — 시스템 파이썬에는 pytest 가 없다.)
  - 프론트 `format:check` / `lint` / `test` (`192 passed`) / `build` / `playwright` (`54 passed`) 전부 통과.
  - Lighthouse 12.8.2 (직전 측정과 같은 버전), Chrome headless. 모바일 7회 · 데스크톱 3회.
- 측정 방법:
  - **배포본이 아니라 로컬에서 쟀다.** 이 브랜치는 아직 안 올라갔으니 배포본으로는 잴 수가 없다. 대신 `main` 과 이 브랜치를 **같은 기계에서 같은 방법으로** 각각 재서 차이를 봤다. `NEXT_PUBLIC_DEMO=1 npm run build && npx next start --port 3200` 로 띄운 로컬 서버 대상.
  - 그래서 이 숫자를 README 의 배포본 표와 나란히 놓고 "좋아졌다"고 말하면 안 된다. 조건이 다르다(네트워크 지연도 CDN 도 없다). 비교해도 되는 것은 아래 두 열 사이뿐이고, README 에도 그렇게 갈라 적었다.
- 결과:

| | 이전 (`main`) | 이후 |
| --- | --- | --- |
| 모바일 Performance | 66 (65–66) | 67 (65–68) |
| 모바일 FCP | 3.66s (2.74–3.77) | 2.87s (2.87–2.88) |
| 모바일 CLS | 0.113 | **0.067** |
| 데스크톱 Performance | 84 (83–84) | **96** |
| 데스크톱 CLS | 0.271 | **0.062** |
| A11y / Best Practices / SEO | 100 | 100 |

- 결정·관찰:
  - **로컬에서 잰 `main` 의 데스크톱 CLS 가 0.271 로 배포본 값과 정확히 같다.** 우연이 아니라, CLS 가 네트워크 조건을 거의 타지 않기 때문이다. 이 항목만은 로컬 수치를 그대로 믿어도 된다는 근거가 생겼다 — 다음부터 CLS 를 확인하려고 배포를 기다릴 필요가 없다.
  - **움직인 것은 점수가 아니라 CLS 다.** 데스크톱 12점은 대부분 CLS 개선이 끌어올린 것이다. 모바일 점수는 두 열 모두 노이즈 범위 안이다.
  - **폰트 작업(T23)의 효과는 FCP 에만 보인다.** 3.66s → 2.87s. 점수로는 거의 드러나지 않는데, 2026-08-22 에 `preconnect` 를 넣었을 때와 같은 양상이다. 렌더 블로킹 요청 하나가 이 앱의 성능 점수를 좌우하는 요인이 아니라는 뜻이다.
  - **로컬 측정은 배포본보다 훨씬 안정적이다.** 배포본 모바일은 49~70 으로 흔들렸는데, 로컬은 65~68 이었다. 네트워크가 빠진 만큼 편차도 빠진 것이다. 편차가 작다고 더 정확한 값은 아니다 — 사용자가 겪는 조건은 배포본 쪽이다.
  - **`main` 모바일 FCP 의 앞 두 회(2.74s, 2.77s)가 뒤 다섯 회보다 1초 빨랐다.** 서버가 막 뜬 직후라 그렇게 보인다. 중앙값을 쓰므로 결과는 3.66s 로 잡혔다. 재측정할 때 서버를 미리 한 번 두드려두는 편이 낫겠다.
- 다음: T28 문서 갱신 — 해소된 빚 정리.

## [T28] 문서와 코드 대조 — 갚은 빚 정리와 틀린 진단 정정
- 일시: 2026-08-28 00:15
- 상태: 완료
- 목적: T23~T27 이 바꾼 것을 문서에 반영하고, 문서가 코드보다 오래 틀려 있던 곳을 찾는다.
- 변경: `docs/ARCHITECTURE.md`, `docs/RETROSPECTIVE.md`, `docs/WORKLOG.md`, `README.md`, `frontend/README.md`
- 검증:
  - 문서 작업이라 테스트는 없다. 대신 **적힌 것을 하나씩 코드와 대조했다.** 아래 "찾은 것"이 그 결과다.
  - `grep -rn "types.ts|scheduler_service|스켈레톤|next/font|렌더 블로킹" --include=*.md` 로 빚 항목이 언급된 자리를 전부 찾아 갱신했다. 다섯 파일에 흩어져 있었다.
- **찾은 것 (문서가 코드보다 오래 틀려 있던 것)**:
  - **`scheduler_service.py` 가 라우터를 역으로 import 한다** — ARCHITECTURE·RETROSPECTIVE 양쪽에 빚 4번으로 올라 있었는데, `grep -rn "from app.routers" backend/app/services/` 결과가 비어 있다. **T18(#38) 에서 이미 사라졌다.** 코드가 고쳐져도 문서는 따라오지 않는다는 예시라 지우지 않고 "갚은 것"에 남겼다.
  - **WORKLOG 의 `남은 작업` 섹션이 T07·T08·T09 를 남은 일로 적고 있었다.** 셋 다 그 사이에 끝났다(스케줄러 안내 → SCHEDULING.md, 알림 → #41, 신호 변화 이력 테이블 → #40). 통째로 다시 썼다.
  - **README 의 프론트 게이트 목록에 `npm test` 가 빠져 있었다.** CI 는 lint 와 build 사이에서 돌리고 있다. 이 문서를 보고 로컬에서 게이트를 돌린 사람은 단위 테스트를 건너뛴 채 "다 통과했다"고 믿게 된다 — T20 에서 내가 그렇게 틀렸었다.
  - **`frontend/README.md` 의 디렉터리 표가 `lib/types.ts` 를 가리키고 있었다.** T25 에서 디렉터리가 됐다.
- 결정:
  - **갚은 빚을 지우지 않고 남겼다.** 무엇이 빚이었는지보다 **왜 그렇게 판단했었는지**가 다음에 또 쓰인다. 특히 CLS 항목은 적어둔 원인이 틀렸던 기록이라, 지우면 그 교훈까지 사라진다.
  - **`next/font` 를 "알려진 빚"에서 "일부러 하지 않은 것"으로 옮겼다.** 오래 "제대로 된 해법"으로 적혀 있었는데, 실제 파일 크기를 재보니 그 길이 더 나쁘다(통짜 2.0MB). 빚이 아니라 판단이다.
  - **회고에 배운 것 7번을 추가했다** — "적어둔 원인은 원인이 아닐 수 있다". 데스크톱 CLS 의 원인을 짐작으로 적어두고 2주를 뒀는데, 재보니 제일 큰 원인이 아니었다. 이 회고에서 제일 값이 나가는 항목이라고 본다.
  - **남은 빚 목록을 4개로 줄여 다시 적었다.** 인증, 폰트 파일 자체 호스팅, 표시 전용 카드 테스트, `PortfolioPanel` 쪼개기.
- **관찰**: 이번 여섯 작업 중 문서만 고친 것이 둘(T27 일부, T28)인데, 그 둘에서 코드 버그와 맞먹는 것을 셋 찾았다. 회고 5번("코드에 제일 가까운 문서가 제일 오래 틀려 있다")이 이번에도 그대로 맞았다.
- 다음: 없음. T23~T28 종료. 남은 것은 아래 목록.

## [T29] 배포본 Lighthouse 재측정
- 일시: 2026-08-27 22:30 (UTC 13:30)
- 상태: 완료
- 목적: T23~T28 이 머지·배포됐다. T27 은 로컬에서만 쟀으므로 배포본 기준 수치를 갱신한다.
- 변경: `README.md` (측정 섹션)
- 검증:
  - Lighthouse 12.8.2 (이전 측정과 같은 버전), Chrome headless, 배포본 URL 직접. 모바일 7회 · 데스크톱 3회.
  - 측정 전 배포 성공 확인: run `33076316680` success, 페이지 HTTP 200, `<link rel="stylesheet">` 둘 다 자기 오리진(jsdelivr 스타일시트 없음).
- 결과 (2026-08-26 → 2026-08-27, 둘 다 배포본):

| | 이전 | 이후 |
| --- | --- | --- |
| 데스크톱 Performance | 83 (83–84) | **97 (97–98)** |
| 데스크톱 CLS | 0.271 | **0.062** |
| 데스크톱 FCP | — | 0.71s |
| 모바일 Performance | 59 (49–70) | 53 (47–60) |
| 모바일 CLS | 미측정 | 0.000 (0.000–0.067) |
| 모바일 FCP | — | 4.15s (3.03–4.29) |
| A11y / Best Practices / SEO | 100 | 100 |

- 결정·판단:
  - **데스크톱은 개선이라고 적었다.** 두 측정의 범위(83–84 vs 97–98)가 겹치지 않는다. 로컬에서 예측한 96 과도 1점 차이다.
  - **모바일 중앙값이 6점 내려갔지만 "나빠졌다"고 적지 않았다.** 범위가 47–60 과 49–70 으로 서로를 덮는다. 이 값은 원래 실행마다 20점 넘게 흔들리고, 7회로도 그 폭 안에서 아무 값이나 나온다. **한쪽으로 유리하게 읽지 않는 것이 이 표의 규칙이고, 불리한 쪽도 같은 규칙을 받는다.**
  - **그래도 왜 갈렸는지는 봤다.** 7회가 두 무리였다 — FCP 3.0초대(점수 56–60)와 4.2초대(점수 47–53). `network-requests` 감사를 까보니 느린 쪽은 스크립트 하나에 220~240ms 씩 걸렸고, 빠른 쪽은 제일 느린 요청이 폰트 54ms 였다. **차이는 코드가 아니라 그때의 네트워크다.** 두 무리 모두 데모 응답 5개를 다 받아 화면을 그렸으므로, 느린 쪽이 "덜 그려서 빨라 보이는" 경우도 아니다.
  - **폰트 작업(T23)이 모바일 점수를 못 올린 이유를 숫자로 확인했다.** 렌더 블로킹 대상이 CDN 스타일시트에서 우리 CSS 청크로 바뀌었을 뿐 전송량이 거의 같다 — 예전 CDN 파일 **12,493B(brotli)**, 지금 우리 청크 **13,057B(gzip)**. 왕복 하나를 줄인 값은 로컬 FCP 로는 보였지만(3.66s → 2.87s) 배포본 점수까지 밀지 못했다. 2026-08-22 `preconnect` 때와 같은 결론이다.
  - **모바일 CLS 0.000 이 "데이터가 안 들어와서 안 흔들린 것"인지 확인했다.** 느린 실행의 `network-requests` 에도 `/api/demo/` 응답 5개(buy-signals, watchlist, portfolio/analysis, stocks/005930/analysis 등)가 다 있다. 화면을 그리고도 안 흔들린 것이 맞다.
- **관찰**: 로컬 측정이 배포본을 어디까지 대신할 수 있는지 이번에 선이 그어졌다. **CLS 와 그것이 끌어올리는 데스크톱 점수는 로컬로 충분하다**(로컬 main 0.271 = 배포본 0.271, 로컬 예측 96 ≈ 배포본 97). **모바일 성능 점수는 안 된다** — 로컬은 65–68 로 조용한데 배포본은 47–60 으로 흔들린다. 네트워크가 빠진 만큼 편차도 빠지지만, 사용자가 겪는 조건은 배포본 쪽이다.
- 다음: 없음.

## 남은 작업 (다음 세션)

*2026-08-27 갱신(T29 시점). 이전 목록은 T07~T09 를 남은 일로 적어두고 있었는데 셋 다 그
사이에 끝나 있었습니다. 아래는 코드와 대조한 것입니다.*

- **백엔드 인증** — 지금은 로컬 전용이라 문제가 아니지만, 공개 IP 에 띄우는 순간 누구나
  남의 보유 종목을 추가·삭제할 수 있다. 호스팅 계획이 생기면 이것부터.
- **폰트 파일 자체 호스팅** — T23 에서 렌더를 막던 스타일시트만 걷어냈다. woff2 조각은
  여전히 jsDelivr 에서 온다. 자체 호스팅하려면 92조각 3.2MB 를 저장소에 넣어야 한다.
  **T29 에서 전송량으로는 얻을 게 없다는 것이 확인됐다** — 남는 이득은 바이트가 아니라
  외부 오리진 하나를 지우는 것뿐이다. 그 값에 3.2MB 를 낼지가 판단할 지점이다.
- **`PortfolioPanel` 쪼개기** — 상태와 표시가 한 컴포넌트에 있어 단위 테스트를 붙이려면
  먼저 갈라야 한다. T26 에서 손대지 않고 남긴 유일한 큰 컴포넌트.
- **표시 전용 카드들의 테스트** — 백엔드 값을 그대로 찍는 카드들은 비어 있다. 지금은
  테스트가 렌더러를 다시 적는 꼴이라 회귀를 못 잡는다. 계산이 생기면 그때 쓴다.
