...
*2026-08-27 갱신. 이전 목록은 T07~T09 를 남은 일로 적어두고 있었는데 셋 다 그 사이에
끝나 있었습니다. 아래는 T28 시점에 코드와 대조한 것입니다.*
...
- **폰트 파일 자체 호스팅** — T23 에서 렌더를 막던 스타일시트만 걷어냈다. woff2 조각은
  여전히 jsDelivr 에서 온다. 자체 호스팅하려면 92조각 3.2MB 를 저장소에 넣어야 하고,
  그만한 값을 하는지는 아직 모른다.
...
- **배포본 Lighthouse 재측정** — T27 은 로컬에서 `main` 과 비교해 쟀다. 이 브랜치가
  머지돼 배포되면 배포본 기준으로 한 번 더 재고 README 의 배포본 표를 갱신한다.
