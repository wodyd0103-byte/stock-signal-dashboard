...
Lighthouse 12.8.2, 모바일 7회 · 데스크톱 3회 중앙값. 괄호 안은 실측 범위입니다.

### 배포본 (2026-08-27)

| | 모바일 | 데스크톱 |
| --- | --- | --- |
| Performance | 53 <sub>(47–60)</sub> | **97** <sub>(97–98)</sub> |
| CLS | 0.000 <sub>(0.000–0.067)</sub> | 0.062 |
| FCP | 4.15s <sub>(3.03–4.29)</sub> | 0.71s |
...
### 직전 측정 대비 (2026-08-26 → 2026-08-27, 둘 다 배포본)

| | 이전 | 이후 | 읽는 법 |
| --- | --- | --- | --- |
| 데스크톱 Performance | 83 <sub>(83–84)</sub> | **97** <sub>(97–98)</sub> | 범위가 겹치지 않습니다 |
| 데스크톱 CLS | 0.271 | **0.062** | 기준(0.1) 통과 |
| 모바일 Performance | 59 <sub>(49–70)</sub> | 53 <sub>(47–60)</sub> | **판단 보류** — 아래 참고 |
| 모바일 CLS | 미측정 | 0.000 | 이전 값이 없습니다 |

**데스크톱은 분명합니다.** 두 측정의 범위가 겹치지 않고, CLS가 0.271에서 0.062로 내려간
만큼이 점수로 올라왔습니다. 로딩 중 레이아웃이 흔들리던 세 원인을 고친 결과입니다
([설계 노트](docs/ARCHITECTURE.md#9-알려진-구조적-빚)의 "갚은 것" 참고).

**모바일 중앙값은 6점 내려갔지만, 그것으로 나빠졌다고 말하지 않습니다.** 두 측정의 범위가
47–60과 49–70으로 서로를 덮습니다. 이 값은 원래 실행마다 20점 넘게 흔들리고, 7회로도 그
폭 안에서 어느 값이든 나올 수 있습니다. 실제로 이번 7회도 두 무리로 갈렸습니다 — FCP 3.0초
근처(점수 56–60)와 4.2초 근처(점수 47–53). 느린 쪽 실행에서는 스크립트 하나가 220~240ms씩
걸렸고, 빠른 쪽에서는 제일 느린 요청이 폰트 54ms였습니다. **차이는 코드가 아니라 그때의
네트워크입니다.** 두 무리 모두 데모 응답 5개를 다 받아 화면을 그렸습니다.

**폰트 작업은 모바일 점수를 올리지 못했습니다.** 렌더를 막던 것이 CDN 스타일시트에서 우리
CSS 청크로 바뀌었을 뿐, 전송량은 거의 같습니다 — 예전 CDN 파일이 12,493바이트(brotli),
지금 우리 청크가 13,057바이트(gzip)입니다. 왕복 하나를 없앤 값은 로컬 측정에서 FCP로
보였지만(3.66초 → 2.87초), 배포본 점수까지 밀어올리지는 못했습니다. 2026-08-22에
`preconnect`를 넣었을 때와 같은 결론입니다.

### 로컬 before/after (2026-08-27)

배포 전에 `main`과 변경분을 **같은 기계에서 같은 방법으로** 재서 변화를 격리한 기록입니다
(`NEXT_PUBLIC_DEMO=1 npm run build && npx next start`). 위의 배포본 숫자와는 조건이 달라
(네트워크 지연도 CDN도 없음) 섞어 읽으면 안 됩니다.

| | 이전 (`main`) | 이후 |
| --- | --- | --- |
| 데스크톱 Performance | 84 <sub>(83–84)</sub> | **96** |
| 데스크톱 CLS | 0.271 | **0.062** |
| 모바일 CLS | 0.113 | **0.067** |
| 모바일 FCP | 3.66s | **2.87s** |
| 모바일 Performance | 66 <sub>(65–66)</sub> | 67 <sub>(65–68)</sub> |

로컬에서 잰 `main`의 데스크톱 CLS 0.271은 배포본 값과 정확히 같았고, 개선 후 예측한 96도
배포본 97로 확인됐습니다. **CLS와 그것이 끌어올리는 데스크톱 점수는 로컬에서 재도 됩니다.**
모바일 성능 점수는 그렇지 않습니다 — 로컬은 65–68로 조용한데 배포본은 47–60으로 흔들립니다.
...
CLS도 이제 Lighthouse를 손으로 돌리지 않아도 지켜집니다.
...
없습니다. 자세한 판단은 [설계 노트](docs/ARCHITECTURE.md#8-일부러-하지-않은-것)에 있습니다.
