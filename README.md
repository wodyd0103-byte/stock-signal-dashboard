...
배포된 데모를 Lighthouse 12.8.2로 잰 값입니다. 2026-08-26 측정 — 모바일 7회, 데스크톱 3회의 중앙값.
...
직전 측정(2026-08-22, 모바일 60 / 데스크톱 84) 이후 리서치 탭에 신호 이력 패널이,
분석 화면에 전환 횟수가 추가됐습니다. 점수는 두 축 모두 노이즈 범위 안에서 그대로입니다.
...
- **데스크톱 CLS 0.271** (0.1 이하가 기준, 이번 측정에서도 같은 값). 분석 화면 스켈레톤이 약 500px인데 실제
